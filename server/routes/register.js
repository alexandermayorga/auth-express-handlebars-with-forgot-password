...
router.post('/', function (req, res, next) {
    ...
    User.findOne({email: user.email},(err,emailDocs)=>{
        if (err) return res.status(404).end();
        if(emailDocs) return res.status(404).json({message: "Email is already in use"});

        User.findOne({ username: user.username }, (err, userNameDocs) => {
            if (err) return res.status(404).end();
            if (userNameDocs) return res.status(404).json({ message: "Username is already in use" });
